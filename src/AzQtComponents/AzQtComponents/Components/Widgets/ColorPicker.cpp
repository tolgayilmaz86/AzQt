    ...
    , m_defaultLForHsMode(0.85)
{
    ...
    connect(m_alphaSlider, &QSlider::valueChanged, this, [this](int alpha) {
        m_currentColorController->setAlpha(static_cast<float>(alpha)/255.0f);
    });
    ...
    connect(m_hueSlider, &QSlider::valueChanged, this, [this](int value) {
        m_currentColorController->setHsvHue(static_cast<float>(value)/360.0f);
    });
    ...
    connect(m_valueSlider, &QSlider::valueChanged, this, [this](int value) {
        m_currentColorController->setValue(static_cast<float>(value)/255.0f);
    });
...
void ColorPicker::applyConfigurationHueSaturation()
{
    ...
    initializeValidation(new HueSaturationValidator(static_cast<float>(m_defaultVForHsMode), this));
}
...
{        
    ...
    else
    {
        ...
        card->setModified(!paletteColors.empty() && !std::ranges::equal(loadedColors, paletteColors));
    }
...
int ColorPicker::colorLibraryIndex(const Palette* palette) const
{
    ...
    return it == m_colorLibraries.end() ? -1 : static_cast<int>(std::distance(m_colorLibraries.begin(), it));
}
...
void ColorPicker::writeSettings() const
{
    ...
    {
        int swatchSize = static_cast<int>(std::distance(swatchSizeActions.begin(), it));
