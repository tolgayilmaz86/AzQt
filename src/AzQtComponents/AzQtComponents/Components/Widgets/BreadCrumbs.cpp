...
namespace AzQtComponents
{
    ...
    QSize BreadCrumbs::sizeHint() const
    {
        ...
        const int noSeparatorsWidth = std::accumulate(fullPath.cbegin(), fullPath.cend(), 0,
        ...
        const auto numIcons = !m_defaultIcon.isEmpty() ? fullPath.size()
            : std::count_if( m_currentPathIcons.cbegin(), m_currentPathIcons.cend(), [](const QString& iconPath) {
