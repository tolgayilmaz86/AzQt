...
{
    m_slider->setValue(static_cast<int>(convertToSliderValue(value)));
}
...
double SliderDouble::calculateRealSliderValue(int value) const
{
    ...
    const double steps = static_cast<double>(numSteps());
...
double SliderDouble::convertPowerCurveValue(double value, bool fromSlider) const
{
    ...
    if (std::abs(m_curveMidpoint - g_defaultCurveMidpoint) < epsilon)
    {
        if (fromSlider)
        {
            return value;
        ...
        {
            return static_cast<int>(normalizedValue * static_cast<double>(m_numSteps));
        }
    }
    ...
    {
        // Calculate the midpoint value of our slider range based on the normalized midpoint curve value
        const double sliderMax = static_cast<double>(m_slider->maximum());
        const double sliderMin = static_cast<double>(m_slider->minimum());
        const double range = sliderMax - sliderMin;
        const double sliderMidpoint = sliderMin + (m_curveMidpoint * range);
        ...
        const double sliderValue = normalizedValue * static_cast<double>(m_numSteps);
