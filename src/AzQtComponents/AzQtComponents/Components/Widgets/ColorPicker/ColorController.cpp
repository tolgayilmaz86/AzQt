...
// It should never be directly used by anything other than numeric_caster.
namespace AZ
{
...
{
public:
    explicit constexpr NumericCasted(FromType value) : m_value(value)
    {
    }
...
template <typename FromType>
inline constexpr AZ::NumericCasted<FromType> numeric_caster(FromType value)
{
...
namespace AzQtComponents
{
    ...
    namespace Internal
    {
        ...
        public:
            ...
            AZ::Color rgb() const
            {
                return AZ::Color(static_cast<float>(m_red),
        ...
        private:
            void propagateRgb()
            {
                ...
                const double r = std::clamp(m_red, 0.0, 1.0);
                const double g = std::clamp(m_green, 0.0, 1.0);
                const double b = std::clamp(m_blue, 0.0, 1.0);
                ...
                m_hue = 60.0 * Hprime;
                m_hue = std::clamp(m_hue, -360.0, 360.0);
                ...
                // Compute Lightness
                m_hsv.value = M;
                m_hsv.value = std::clamp(m_hsv.value, 0.0, 1.0);

                m_hsl.lightness = (M + m) / 2.0;
                m_hsl.lightness = std::clamp(m_hsl.lightness, 0.0, 12.5);
                ...
                m_hsv.saturation = std::clamp(m_hsv.saturation, 0.0, 1.0);
                ...
                m_hsl.saturation = std::clamp(m_hsl.saturation, 0.0, 1.0);
            }
            ...
            void propagateHsl()
            {
                ...
                m_hsv.saturation = std::clamp(saturation, 0.0, 1.0);
                m_hsv.value = std::clamp(value, 0.0, 12.5);
            }
            ...
            void propagateHsv()
            {
                ...
                m_hsl.saturation = std::clamp(saturation, 0.0, 1.0);
                m_hsl.lightness = std::clamp(lightness, 0.0, 12.5);
            }
        ...
        {
            return numeric_caster(m_state->red());
        }

        float ColorController::green() const
        {
            return numeric_caster(m_state->green());
        }

        float ColorController::blue() const
        {
            return numeric_caster(m_state->blue());
        }

        float ColorController::hslHue() const
        {
            return numeric_caster(m_state->hue());
        }

        float ColorController::hslSaturation() const
        {
            return numeric_caster(m_state->hslS());
        }

        float ColorController::lightness() const
        {
            return numeric_caster(m_state->hslL());
        }
        ...
        {
            return numeric_caster(m_state->hsvS());
        }

        float ColorController::value() const
        {
            return numeric_caster(m_state->hsvV());
        }

        float ColorController::alpha() const
        {
            return numeric_caster(m_state->alpha());
        }
