...
```bash
docker build -f ./.devcontainer/Dockerfile --tag=AzQt:latest .
docker run -it AzQt:latest
```
...
```bash
docker build --tag=AzQt:latest --build-arg GCC_VER=10 --build-arg LLVM_VER=11 .
```
...
```bash
docker build --tag=AzQt:latest --build-arg USE_CLANG=1 .
```
